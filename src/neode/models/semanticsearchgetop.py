"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
from .entity import Entity, EntityTypedDict
from .graph import Graph, GraphTypedDict
from .triple import Triple, TripleTypedDict
from neode.types import BaseModel, UNSET_SENTINEL
from neode.utils import FieldMetadata, QueryParamMetadata
from pydantic import model_serializer
from typing import List, Optional
from typing_extensions import Annotated, NotRequired, TypedDict


class SemanticSearchGetRequestTypedDict(TypedDict):
    q: str
    r"""Natural language search query"""
    types: NotRequired[str]
    r"""Comma-separated types to search (entities,triples,graphs)"""
    index_id: NotRequired[str]
    r"""Filter results to a specific index"""
    limit: NotRequired[int]
    r"""Maximum results per type (default 20, max 100)"""
    threshold: NotRequired[float]
    r"""Minimum similarity score 0-1 (default 0.3)"""


class SemanticSearchGetRequest(BaseModel):
    q: Annotated[
        str, FieldMetadata(query=QueryParamMetadata(style="form", explode=True))
    ]
    r"""Natural language search query"""

    types: Annotated[
        Optional[str],
        FieldMetadata(query=QueryParamMetadata(style="form", explode=True)),
    ] = "entities,triples"
    r"""Comma-separated types to search (entities,triples,graphs)"""

    index_id: Annotated[
        Optional[str],
        FieldMetadata(query=QueryParamMetadata(style="form", explode=True)),
    ] = None
    r"""Filter results to a specific index"""

    limit: Annotated[
        Optional[int],
        FieldMetadata(query=QueryParamMetadata(style="form", explode=True)),
    ] = 20
    r"""Maximum results per type (default 20, max 100)"""

    threshold: Annotated[
        Optional[float],
        FieldMetadata(query=QueryParamMetadata(style="form", explode=True)),
    ] = 0.3
    r"""Minimum similarity score 0-1 (default 0.3)"""

    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = set(["types", "index_id", "limit", "threshold"])
        serialized = handler(self)
        m = {}

        for n, f in type(self).model_fields.items():
            k = f.alias or n
            val = serialized.get(k)

            if val != UNSET_SENTINEL:
                if val is not None or k not in optional_fields:
                    m[k] = val

        return m


class SemanticSearchGetDataTypedDict(TypedDict):
    entities: NotRequired[List[EntityTypedDict]]
    triples: NotRequired[List[TripleTypedDict]]
    graphs: NotRequired[List[GraphTypedDict]]


class SemanticSearchGetData(BaseModel):
    entities: Optional[List[Entity]] = None

    triples: Optional[List[Triple]] = None

    graphs: Optional[List[Graph]] = None

    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = set(["entities", "triples", "graphs"])
        serialized = handler(self)
        m = {}

        for n, f in type(self).model_fields.items():
            k = f.alias or n
            val = serialized.get(k)

            if val != UNSET_SENTINEL:
                if val is not None or k not in optional_fields:
                    m[k] = val

        return m


class SemanticSearchGetCountTypedDict(TypedDict):
    pass


class SemanticSearchGetCount(BaseModel):
    pass


class SemanticSearchGetResponseTypedDict(TypedDict):
    r"""Search results"""

    success: NotRequired[bool]
    data: NotRequired[SemanticSearchGetDataTypedDict]
    count: NotRequired[SemanticSearchGetCountTypedDict]


class SemanticSearchGetResponse(BaseModel):
    r"""Search results"""

    success: Optional[bool] = None

    data: Optional[SemanticSearchGetData] = None

    count: Optional[SemanticSearchGetCount] = None

    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = set(["success", "data", "count"])
        serialized = handler(self)
        m = {}

        for n, f in type(self).model_fields.items():
            k = f.alias or n
            val = serialized.get(k)

            if val != UNSET_SENTINEL:
                if val is not None or k not in optional_fields:
                    m[k] = val

        return m
